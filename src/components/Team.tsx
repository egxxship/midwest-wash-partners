      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {team.map((member) => (
            <div
              key={member.name}
              className="group rounded-2xl border border-stone-200 bg-stone-50 overflow-hidden hover:border-stone-300 hover:shadow-sm transition-all duration-300"
            >
              {/* Avatar placeholder */}
              <div className="h-48 bg-gradient-to-br from-stone-200 to-stone-100 flex items-center justify-center">
                <div className="h-24 w-24 rounded-full bg-white border-2 border-stone-200 flex items-center justify-center">
              ...
              <div className="p-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {member.specialties.map((spec) => (
                    <span
                      key={spec}
                      className="inline-flex rounded-full bg-white border border-stone-200 px-3 py-1 text-xs text-stone-500"
                    >
                      {spec}
                    </span>
                  ))}
